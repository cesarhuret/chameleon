...
int8_t PixyController::init(int8_t targetSignature, int16_t thresholdHeight, int16_t thresholdWidth, int16_t thresholdX, int16_t thresholdY)
{
...
{
    std::vector<Block> blocks = pixy.getBlocks();
...
bool PixyController::isBall(Block block)
{
    ...
    if (abs(block.x - PIXY_CAM_WIDTH / 2) > thresholdX || abs(block.y - PIXY_CAM_HEIGHT / 2) > thresholdY)
    {
        return false;
    }
    return true;
